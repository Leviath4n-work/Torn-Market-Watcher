...
// @namespace    lewis.torn.marketwatch.v5.6
// @version      6.2.0
...
(function () {
  ...
  const POPUP_HISTORY_TTL_MS = 10 * 60 * 1000;
...
function clearPopupHistory() {
  ...
  function makeFingerprint(itemRule, listing) {
    return `${itemRule.itemId}|${extractPrice(listing)}|${extractArmorRaw(listing) ?? 'noa'}|${extractQualityRaw(listing) ?? 'noq'}`;
  }
