...
// @namespace    leviath4n.torn.marketwatch.v6.5.8
// @version      6.5.8
...
(function () {
  ...
  */
  ...
  const BACKEND_BASE_URL = 'http://146.190.216.11:3000';
  const MEMBERSHIP_TRIAL_MESSAGE = 'Initial API signup comes with 1 day membership to trial use.';
  const MEMBERSHIP_PAYMENT_MESSAGE = 'Send 1 Xanax to Leviathan [3634894] to get 5 days membership.';
  ...
  const POPUP_HISTORY_TTL_MS = 3 * 60 * 60 * 1000;
  const POPUP_HISTORY_MAX = 100;

  const MARKET_TAX_RATE = 0.05;
  const COMP_UNDERCUT = 1;
  const STEAL_COMP_MAX_MULTIPLIER = 1.18;
  const ALLOW_FALLBACK_COMP_ESTIMATE = true;
...
`;
  ...
  function getStoredPlayerId() {
    return localStorage.getItem(MEMBERSHIP_KEYS.playerId) || '';
  }

  function setStoredPlayerId(playerId) {
    localStorage.setItem(MEMBERSHIP_KEYS.playerId, String(playerId || ''));
  }

  function getStoredPlayerName() {
    return localStorage.getItem(MEMBERSHIP_KEYS.playerName) || '';
  }

  function setStoredPlayerName(playerName) {
    localStorage.setItem(MEMBERSHIP_KEYS.playerName, String(playerName || ''));
  }

  function getStoredApiKey() {
    return localStorage.getItem(MEMBERSHIP_KEYS.apiKey) || '';
  }

  function setStoredApiKey(apiKey) {
    localStorage.setItem(MEMBERSHIP_KEYS.apiKey, String(apiKey || ''));
  }
  ...
  function getLastAuthStatus() {
    return getJson(MEMBERSHIP_KEYS.lastAuthStatus, null);
  }

  function setLastAuthStatus(status) {
    setJson(MEMBERSHIP_KEYS.lastAuthStatus, status || null);
  }

  
  function clearStoredMembership() {
    localStorage.removeItem(MEMBERSHIP_KEYS.playerId);
    localStorage.removeItem(MEMBERSHIP_KEYS.playerName);
    localStorage.removeItem(MEMBERSHIP_KEYS.apiKey);
    localStorage.removeItem(MEMBERSHIP_KEYS.lastAuthStatus);
...
function loadVelocityMap() {
  return getJson(STORAGE_KEYS.velocity, {});
}

function saveVelocityMap(map) {
  setJson(STORAGE_KEYS.velocity, map);
}
...
function loadScanStatusMap() {
  return getJson(STORAGE_KEYS.scanStatus, {});
}

function saveScanStatusMap(map) {
  setJson(STORAGE_KEYS.scanStatus, map);
}
...
function loadPopupHistory() {
  return getJson(STORAGE_KEYS.popupHistory, []);
}

function savePopupHistory(entries) {
  setJson(STORAGE_KEYS.popupHistory, entries);
}
...
function getSettings() {
  ...
  function saveWatchlist(list) {
    setJson(STORAGE_KEYS.watchlist, list);
    refreshDebugPanel(true);
  }
  ...
  function ensureBadge() {
    ...
    badgeEl.style.boxShadow = '0 2px 10px rgba(0,0,0,0.35)';
    ...
    debugToggleEl.style.background = 'rgba(20,20,20,0.96)';
    debugToggleEl.style.boxShadow = '0 2px 10px rgba(0,0,0,0.35)';
    ...
    debugToggleEl.addEventListener('click', handleDebugToggle);
...
function soundForTier(tier) {
  ...
  function appendDebugSection(container, key, titleText, renderContent, options = {}) {
    ...
    chevron.textContent = isDebugSectionCollapsed(key) ? '+' : 'â';
    ...
    if (scanStatus) {
      ...
      if (Array.isArray(scanStatus.pageDetails) && scanStatus.pageDetails.length) {
        ...
        scanStatus.pageDetails.slice(0, 5).forEach(detail => {
          ...
          detailLine.textContent = `P${detail.page}: ${detail.count} listings | ${shortSig.length > 54 ? `${shortSig.slice(0, 54)}â¦` : shortSig}`;
  ...
  function refreshDebugPanel(force = false) {
    ...
    const lastAlert = getStampedMessage(STORAGE_KEYS.lastAlert);
    const lastError = getStampedMessage(STORAGE_KEYS.lastError);
    const lastScanAt = getNumber(STORAGE_KEYS.lastScanAt, 0);
    const lastValueFetch = getNumber(STORAGE_KEYS.lastValueFetch, 0);
    const marketValues = getJson(STORAGE_KEYS.marketValues, {});
    const scanStatusMap = loadScanStatusMap();
    ...
    const apiEstimate = estimateApiUsagePerMinute();
    addDebugLine(statusBlock, 'Enabled', isEnabled() ? 'yes' : 'no');
    addDebugLine(statusBlock, 'Leader', isLeader ? 'yes' : 'no');
    addDebugLine(statusBlock, 'Running loop', isRunningLoop ? 'yes' : 'no');
    ...
    appendDebugSection(debugPanelEl, 'popup_history', `Popup history (last 3h, ${popupHistory.length})`, (section) => {
      ...
      if (popupHistory.length === 0) {
        ...
        popupHistory.forEach(entry => {
          ...
          meta.textContent = `${formatDateTime(entry.at)}${Number(entry.count || 1) > 1 ? ` â¢ x${Number(entry.count || 1)}` : ''}`;
  ...
  function loadSeenMap() {
    return getJson(STORAGE_KEYS.seenMap, {});
  }
  ...
  if (shouldSuppressPopupAlert(match, formatted, url)) {
    return false;
  }
      ...
      for (const itemRule of watchlist) {
        ...
        try {
          ...
          if (result && result.listing) {
            const lastSeenAt = Number(seenMap[result.fingerprint] || 0);
            if (!lastSeenAt || (now() - lastSeenAt) >= settings.alertCooldownMs) {
              seenMap[result.fingerprint] = now();
              notifyWatchItemHit(result, marketValues);
            }
          }
        } catch (err) {
