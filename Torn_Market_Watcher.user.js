...
(function () {
  ...
  // This review build intentionally has no upstream auto-update URL.
  ...
  function getScriptVersion() {
    try {
      return (typeof GM_info !== 'undefined' && GM_info?.script?.version) ? GM_info.script.version : '7.0.0';
    } catch {
      return '7.0.0';
    }
  }
...
function getWatchlist() {
  ...
  function updateBadge() {
    ...
    badgeEl.append(dot, document.createTextNode('Market Watcher'));
...
function soundForTier(tier) {
  ...
  function showToast(title, text, tier, onOpen) {
    ...
    copy.setAttribute('aria-label', 'Open ' + title + ' on the item market');
...
function deletePreset(name) {
  ...
  const designState = { tab: 'watchlist', edit: null, search: '', notice: '', noticeError: false, busy: false, tools: false, importText: '', importMode: 'merge' };
  ...
  function uiStatus() {
    ...
    if (!getWatchlist().some(rule => rule.enabled)) return { label: 'Ready when you are', detail: 'Add an item below or enable a saved item to begin.', tone: 'idle' };
  ...
  function renderWatchlistView(container, model) {
    ...
    for (const [label, value, note] of [
      ['ACTIVE ITEMS', model.watchlist.filter(r => r.enabled).length, 'of ' + model.watchlist.length + ' saved'],
    ...
    const drawResults = () => {
      ...
      for(const item of found) {
        const row=uiButton('',()=>{
          const list=getWatchlist(); if(!list.some(r=>r.itemId===item.itemId)) list.push(sanitizeImportedWatchRule({...item,id:'watch_'+item.itemId+'_'+Date.now()}));
    ...
    for(const rule of model.watchlist) renderItemCard(cards,rule,model);
    container.append(cards);
  }

  function renderItemCard(container,rule,model) {
    const card=uiNode('article','umw-item');card.dataset.enabled=String(rule.enabled);
    ...
    chips.append(uiNode('span','umw-chip',rule.useMV ? 'Price ≤ '+Math.round(rule.maxMultiplier*100)+'% of market value' : 'No price limit'));
    ...
    bottom.append(uiNode('span','umw-muted',!rule.enabled?'Paused':status?.ok===false?'Scan issue · '+status.errorMessage:status?.at?'Checked '+formatElapsedSince(status.at)+' · '+Number(status.listingsScanned||0)+' listings':'Waiting for first scan'));
  ...
  function renderAlertsView(container,model) {
    ...
    for(const entry of model.popupHistory){
      ...
      row.append(title,uiNode('strong','umw-price',uiMoney(entry.price)));card.append(row,uiNode('p','umw-muted',entry.text||''));
...
function extractListingIdentity(listing) {
  ...
  async function scanCycle() {
    ...
    try {
      const settings = getSettings();
      const watchlist = getWatchlist();
      if (!watchlist.some(rule => rule.enabled)) return;
      let marketValues = getJson(STORAGE_KEYS.marketValues, {});
      if (watchlist.some(rule => rule.enabled && rule.useMV)) {
      ...
      for (const itemRule of watchlist) {
        ...
        if (itemRule.useMV && !(Number(marketValues[itemRule.itemId]) > 0)) {
          setScanStatus(itemRule.itemId, { ok: false, matchFound: false, errorMessage: 'Current market value unavailable' });
          continue;
        }
        ...
        try {
          ...
          for (const listing of result.matches || []) {
            assertScan(revision);
            const match = { ...result, listing, fingerprint: makeFingerprint(itemRule, listing) };
            ...
            notifyWatchItemHit(match, payload);
            recordAlertDispatch(match, payload, seenMap, decision);
            saveSeenMap(seenMap);
            // One new match per item per cycle limits noise; other new matches remain eligible.
            break;
          }
        } catch (err) {
  ...
  async function init() {
    ...
    window.addEventListener('storage', event => {
      if (event.key === null || [STORAGE_KEYS.enabled, STORAGE_KEYS.settings, STORAGE_KEYS.watchlist,
        MEMBERSHIP_KEYS.playerId, MEMBERSHIP_KEYS.apiKey, MEMBERSHIP_KEYS.lastAuthStatus, LOCK_KEY].includes(event.key)) {
