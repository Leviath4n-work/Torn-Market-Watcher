...
// @namespace    leviath4n.torn.marketwatch.v6.7.1
// @version      6.7.1
// @description  Multi-item Torn market watcher with presets, smarter popup dedupe, flexible item catalog parsing, refactored state/render pipeline, server-gated membership, stored user API scanning, watchlists, debug menu, tiers, sound, vibration, persistent popups, and armor/quality filters
// @author       Leviath4n
...
function addPopupHistoryEntry(entry) {
  ...
  const normalized = {
    ...
    armor: Number.isFinite(Number(entry?.armor)) ? Number(entry?.armor) : null,
    quality: Number.isFinite(Number(entry?.quality)) ? Number(entry?.quality) : null,
...
function buildAlertIdentity(match) {
  const itemId = Number(match?.itemRule?.itemId || 0);
  ...
  if (listingIdentity) {
    return `${itemId}|listing:${listingIdentity}`;
  }

  return [
    itemId,
    `seller:${sellerIdentity || 'noseller'}`,
    `price:${price || 0}`,
    `armor:${Number.isFinite(armor) ? armor.toFixed(2) : 'na'}`,
    `quality:${Number.isFinite(quality) ? quality.toFixed(2) : 'na'}`
  ].join('|');
}
...
function shouldSuppressPopupAlert(match, payload) {
  const normalizedFingerprint = String(payload?.fingerprint || match?.fingerprint || '').trim();
  const alertIdentity = String(payload?.alertIdentity || buildAlertIdentity(match) || '').trim();

  if (hasRecentPopupAlert(alertIdentity, normalizedFingerprint)) {
    return true;
  }

  const entries = prunePopupHistory(loadPopupHistory());
  const normalizedUrl = String(payload?.url || '').trim();
  const normalizedItemId = Number(payload?.itemRule?.itemId || match?.itemRule?.itemId || 0);
  const normalizedPrice = Number(payload?.price || 0);
  const normalizedSellerIdentity = String(payload?.sellerIdentity || '').trim();

  return entries.some(entry =>
    Number(entry?.itemId || 0) === normalizedItemId &&
    Number(entry?.price || 0) === normalizedPrice &&
    String(entry?.sellerIdentity || '').trim() === normalizedSellerIdentity &&
...
function soundForTier(tier) {
  ...
  function refreshDebugPanel(force = false) {
    ...
    appendDebugSection(debugPanelEl, 'watchlist', 'Watchlist', (section) => {
      ...
      const presetsShell = createSectionShell(section, 'Saved presets', {
        padding: '8px',
        marginBottom: '8px',
      ...
      presetTopRow.style.display = 'flex';
      presetTopRow.style.gap = '6px';
      presetTopRow.style.flexWrap = 'wrap';
      presetTopRow.style.alignItems = 'center';
      presetTopRow.style.marginBottom = presets.length ? '8px' : '0';

      const presetSelect = document.createElement('select');
      styleFormControl(presetSelect);
      presetSelect.style.flex = '1 1 180px';

      const presetPlaceholder = document.createElement('option');
      presetPlaceholder.value = '';
      presetPlaceholder.textContent = presets.length ? 'Select a preset...' : 'No presets saved yet';
      presetSelect.appendChild(presetPlaceholder);

      presets.forEach(preset => {
        const option = document.createElement('option');
        option.value = preset.name;
        const itemCount = Array.isArray(preset.watchlist) ? preset.watchlist.length : 0;
        option.textContent = `${preset.name} (${itemCount})`;
        presetSelect.appendChild(option);
      });

      const savePresetBtn = createActionButton('Save current', () => {
        const suggestedName = presetSelect.value || '';
        const rawName = window.prompt('Save current watchlist as preset:', suggestedName);
        if (!rawName) return;

        const safeName = sanitizePresetName(rawName);
        if (!safeName) return;

        if (presets.some(preset => preset.name === safeName)) {
          const overwrite = window.confirm(`Overwrite preset "${safeName}"?`);
          if (!overwrite) return;
        }

        try {
          saveCurrentWatchlistAsPreset(safeName);
        } catch (err) {
          alert(`Preset save failed: ${err?.message || err}`);
        }
      }, {
        background: UI_THEME.primaryBtnBg,
        border: UI_THEME.primaryBtnBorder
      });

      const loadPresetBtn = createActionButton('Load', () => {
        ...
        try {
          loadPresetIntoWatchlist(selectedName, 'replace');
        } catch (err) {
          alert(`Preset load failed: ${err?.message || err}`);
        }
      });

      const mergePresetBtn = createActionButton('Merge', () => {
        const selectedName = presetSelect.value;
        if (!selectedName) {
          alert('Choose a preset first.');
          return;
        ...
        try {
          loadPresetIntoWatchlist(selectedName, 'merge');
        } catch (err) {
          alert(`Preset merge failed: ${err?.message || err}`);
        }
      });

      const deletePresetBtn = createActionButton('Delete', () => {
        const selectedName = presetSelect.value;
        if (!selectedName) {
          alert('Choose a preset first.');
          return;
        ...
        const confirmed = window.confirm(`Delete preset "${selectedName}"?`);
        if (!confirmed) return;
        deletePreset(selectedName);
      }, {
        background: UI_THEME.dangerBtnBg,
        border: UI_THEME.dangerBtnBorder
      });

      presetTopRow.appendChild(presetSelect);
      presetTopRow.appendChild(savePresetBtn);
      presetTopRow.appendChild(loadPresetBtn);
      presetTopRow.appendChild(mergePresetBtn);
      presetTopRow.appendChild(deletePresetBtn);
      presetsShell.appendChild(presetTopRow);

      if (presets.length) {
        const presetMeta = document.createElement('div');
        presetMeta.style.fontSize = '10px';
        presetMeta.style.opacity = '0.72';
        presetMeta.textContent = 'Load replaces your watchlist. Merge adds preset items without duplicating matching item/name pairs.';
        presetsShell.appendChild(presetMeta);
      } else {
        const presetEmpty = document.createElement('div');
        presetEmpty.style.fontSize = '10px';
        presetEmpty.style.opacity = '0.72';
        presetEmpty.textContent = 'Save your current watchlist as a preset to reuse it on this device instantly.';
        presetsShell.appendChild(presetEmpty);
      }
